...
pure real(pr) function integral(f,a,b)
	! Интегрирование ведется по методу Гаусса
	implicit none

	real(pr), intent(in) :: a, b
	interface
		pure real(pr) function f(x)
			use init, only: pr => pr
			real(pr), intent(in) :: x
		end function f
	end interface
...
pure subroutine make_gauss_coeffs(n,A,t)
	...
	if (n == 5) then
		t(1) = real(0.000000000000000,kind=pr)
		A(1) = real(0.568888888888889,kind=pr)

		t(2) = real(0.538469310105683,kind=pr)
		A(2) = real(0.478628670499366,kind=pr)
		t(3) = -t(1)
		A(3) = -A(1)

		t(4) = real(0.906179845938664,kind=pr)
		A(4) = real(0.236926885056189,kind=pr)
		t(5) = -t(3)
		A(5) = -A(3)
	else 
