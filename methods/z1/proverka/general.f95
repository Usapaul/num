program diffur
use funs

implicit none
integer :: n, i
real(8) :: h, kap1, nu1, kap2, nu2
real(8) :: r, z
real(8), allocatable :: Y(:), X(:), P(:), Q(:), F(:)
real(8), allocatable :: kf(:,:), U(:), V(:)
character(12) :: nchar
!---------------------------------------
write(*,*) 'input N'
read(*,*) n
h=(b-a)/n
allocate (Y(0:n), X(0:n), P(0:n), Q(0:n), F(0:n))
forall (i=0:n) X(i)=a+i*h
forall (i=0:n)
P(i)=Pfun(X(i))
Q(i)=Qfun(X(i))
F(i)=Ffun(X(i))
end forall
!---------------------------------------
r=(4.0d0-2.0d0*(2.0d0-Q(1)*h**2)/(2.0d0+P(1)*h))*alp2
z=2.0d0*alp1*h-3.0d0*alp2-alp2*(P(1)*h-2.0d0)/(2.0d0+P(1)*h)
kap1=-r/z
nu1=(2.0d0*h*Abig+2.0d0*alp2*F(1)*h**2/(2.0d0+P(1)*h))/z

r=(-4.0d0+2.0d0*(2.0d0-Q(n-1)*h**2)/(2.0d0-P(n-1)*h))*bet2
z=2.0d0*bet1*h+3.0d0*bet2-bet2*(2.0d0+P(n-1)*h)/(2.0d0-P(n-1)*h)
kap2=-r/z
nu2=(2.0d0*h*Bbig-2.0d0*bet2*F(n-1)*h**2/(2.0d0-P(n-1)*h))/z
!---------------------------------------
allocate (kf(4,1:n-1), U(0:n), V(0:n))
forall (i=1:n-1)
kf(1,i)=1.0d0+h/2.0d0*P(i)
kf(2,i)=2.0d0-h**2*Q(i)
kf(3,i)=1.0d0-h/2.0d0*P(i)
kf(4,i)=h**2*F(i)
end forall

U(0)=kap1; V(0)=nu1
do i=1,n-1
    U(i)=kf(1,i)/(kf(2,i)-kf(3,i)*U(i-1))
    V(i)=(kf(3,i)*V(i-1)-kf(4,i))/(kf(2,i)-kf(3,i)*U(i-1))
enddo
!---------------------------------------
Y(n)=(-kap2*V(n-1)-nu2)/(kap2*U(n-1)-1.0d0)
do i=n-1,0,-1
    Y(i)=U(i)*Y(i+1)+V(i)
enddo

open(777,file='yyy.dat')
!open(770,file='ynox.dat')
do i=0,n
    write(777,*) X(i), Y(i)
!    write(770,*) Y(i)
enddo
!close(770)
close(777)

open(888,file='ggg.dat')
!open(880,file='gnox.dat')
do i=0,n
    write(888,*) X(i), G(X(i))
!    write(880,*) G(X(i))
enddo
!close(880)
close(888)

open(550,file='gminusy.dat')
do i=0,n
    write(550,*) X(i), Y(i)-G(X(i))
enddo
close(550)

write(nchar,*) n
write(*,*) n, h, Y(n/2)-G(X(n/2))
open(440,file='handd'//nchar//'.dat')
    write(440,*) h, Y(n/2)-G(X(n/2))
close(440)


end program diffur
